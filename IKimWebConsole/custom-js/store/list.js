$(function () {
    $(document).off('click', '.add-store').on('click', '.add-store', fnCreate);
    ...
    $(document).off('click', '.icon-address').on('click', '.icon-address', function (e) {
        e.preventDefault();        
        fnGetAddress($(this).data('id'));
    });
...
function fnGetAddress(id) {
    $('#loading-element').show();
    $.ajax({
        url: `/Store/GetAddress/${id}`,
        type: "Get",
        dataType: "html",
        success: function (responseData) {
            $("#divAddress").empty().html(responseData);
...
function fnDelete(id) {
    fnHandleAjaxRequest({
        ...
        onError: function () {
            showToastr("Failed to delete store.");
        }
    });
}
